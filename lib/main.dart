...
import 'data/services/screen_protection_service.dart';
...
void main() async {
  ...
  // Initialize Firebase
  try {
    // On iOS, Firebase is configured natively in AppDelegate.swift
    // On Android, we provide explicit options
    if (Platform.isIOS) {
      try {
        await Firebase.initializeApp();
      } catch (e) {
        // Firebase may already be initialized natively — that's fine
        print('iOS Firebase.initializeApp note: $e');
      }
    } else {
      await Firebase.initializeApp(
    ...
    // Initialize Theme Service first (critical for UI — must be awaited)
    await ThemeService.initialize();

    // Initialize FCM and Notifications in background — must NOT block startup
    // on iOS these await permission dialogs which would freeze the white screen
    Future(() async {
    ...
    // Still try to initialize theme even if Firebase fails
    try {
      await ThemeService.initialize();
    } catch (_) {}
  }

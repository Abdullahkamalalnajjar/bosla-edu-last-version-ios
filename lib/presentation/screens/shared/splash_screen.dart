...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    // Initialize screen protection after the widget is mounted to avoid iOS native crash
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        final screenProtection = ScreenProtectionService();
        await screenProtection.enableProtection();
        await screenProtection.startMonitoring();
      } catch (e) {
        debugPrint('Warning: Screen protection initialization failed: $e');
      }
    });
  }
  ...
  Future<Widget> _performAuthCheck() async {
    try {
      ...
      // Update FCM token on backend — do this in background with delay
      // to ensure APNS token is fully ready on iOS before we try
      Future.delayed(const Duration(seconds: 3), () async {
        print('🔄 Attempting FCM token refresh and backend update...');

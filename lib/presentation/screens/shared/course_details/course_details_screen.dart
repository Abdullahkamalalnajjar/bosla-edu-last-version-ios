...
class _CourseDetailsScreenState extends State<CourseDetailsScreen>
  ...
  bool _isRefreshing = false; // Track loading state for skeleton
  bool _isScoreLoading = true; // Track score card loading state
  ...
  void initState() {
    ...
    // If initially empty, show skeleton and refresh
    if (_lectures.isEmpty ||
        (widget.initialLectureId != null &&
            !_lectures.any((l) => l.id == widget.initialLectureId))) {
      _isRefreshing = true; // Show skeleton immediately
      _refreshCourse();
    }
  ...
  Future<void> _fetchStudentScore() async {
    ...
    if (studentId == null) {
      if (mounted) setState(() => _isScoreLoading = false);
      return;
    }
            ...
            if (!widget.isTeacher)
              SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                  child: _isScoreLoading
                      ? _buildScoreCardSkeleton()
                      : _studentScore != null
                          ? StudentScoreCard(studentScore: _studentScore!)
  ...
  Widget _buildScoreCardSkeleton() {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final shimmer = (t < 0.5 ? t * 2 : (1 - t) * 2);
        final c = Color.lerp(baseColor, shimmerColor, shimmer)!;
  ...
  Widget _buildSkeletonLoader() {
    ...
    return ListView(
      ...
      children: List.generate(4, (i) => _buildSkeletonCard(isDark, i)),
    );
  }
  ...
  Widget _buildSkeletonCard(bool isDark, int index) {
    ...
    return AnimatedBuilder(
      animation: _backgroundController,
      builder: (context, _) {
        final t = (_backgroundController.value + index * 0.2) % 1.0;
        final shimmer = (t < 0.5 ? t * 2 : (1 - t) * 2);
        final mixedColor = Color.lerp(baseColor, shimmerColor, shimmer)!;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  height: 1,
                  color: mixedColor.withOpacity(0.5),
                ),
                const SizedBox(height: 12),
                // 2 material pills
                Row(
                  children: [
                    _skeletonPill(mixedColor, 80),
                    const SizedBox(width: 8),
                    _skeletonPill(mixedColor, 100),
                    const SizedBox(width: 8),
                    _skeletonPill(mixedColor, 60),
                  ],
                ),
              ],
            ),
  ...
  Widget _skeletonPill(Color color, double width) {
    return Container(
      width: width,
      height: 30,
      decoration: BoxDecoration(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // Subtitle
                Text(
                  'لم يتم إضافة أي محاضرات لهذا الكورس حتى الآن.\nتابع الصفحة للحصول على أحدث المحتوى.',
                  style: GoogleFonts.outfit(
                    fontSize: 14,
                    height: 1.6,
                    color: isDark

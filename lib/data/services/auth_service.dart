...
class AuthService {
  ...
  Future<ApiResponse<AuthResponse>> login(
    ...
    try {
      ...
      try {
        ...
        fcmToken ??= await FcmService.getCachedFcmToken();
      } catch (e) {
        print('Warning: Could not get FCM token: $e');
      }
      // If empty string, treat as null so it's NOT sent (empty string worse than null)
      if (fcmToken != null && fcmToken.isEmpty) fcmToken = null;
      print('📱 FCM token for signin: ${fcmToken != null ? fcmToken!.substring(0, 20) + "..." : "NOT AVAILABLE — will be sent as null"}');
      ...
      print('URL: ${ApiConstants.signin}');
      print('Method: POST');
      print('Headers: ${ApiConstants.headers}');
      print('Body: $requestBody');
      print('FCM token: ${fcmToken?.isNotEmpty == true ? fcmToken!.substring(0, 20) + "..." : "EMPTY"}');
      print('=================================');
  ...
  Future<ApiResponse<AuthResponse>> signInWithGoogle(String idToken) async {
    try {
      // Get FCM token - same logic as regular signin
      String? fcmToken;
      try {
        fcmToken = await FcmService.getFcmToken();
        fcmToken ??= await FcmService.getCachedFcmToken();
      } catch (e) {
        print('Warning: Could not get FCM token for Google signin: $e');
      }
      if (fcmToken != null && fcmToken.isEmpty) fcmToken = null;
      print('📱 Google Signin FCM token: ${fcmToken != null ? fcmToken!.substring(0, 20) + "..." : "NOT AVAILABLE"}');

      final queryParams = <String, String>{'idToken': idToken};
      if (fcmToken != null) queryParams['FCMtoken'] = fcmToken!;

...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  // Always show our local notification in background to ensure sound plays.
  // The local notification has sound configured regardless of FCM payload settings.
...
class NotificationService {
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      print('🔑 Current FCM Token in NotificationService: $token');
    } catch (e) {
      print('⚠️ Could not get FCM token in NotificationService: $e');
    }
    ...
    if (role != null) {
      ...
      print('⚠️ No role found — skipping topic subscription');
    }
    ...
    if (initialMessage != null) {
      ...
      print('🔔 No initial notification message');
    }
  ...
  static Future<void> _showLocalNotification(RemoteMessage message) async {
    ...
    const iosDetails = DarwinNotificationDetails(
      ...
      presentBadge: true,
      presentSound: true,
      // sound: null → plays iPhone's default notification sound
    );
  ...
  static void _handleForegroundMessage(RemoteMessage message) async {
    ...
    if (role != null && !_isNotificationForRole(message.data, role)) {
      print('🚫 FILTERED: Notification type "${message.data["type"]}" not meant for role "$role"');
      return;
    }

    // Show notification based on message type:
    // - iOS with notification payload → AppDelegate willPresent already handles it with sound
    //   (completionHandler([.alert, .badge, .sound])) — no need to duplicate
    // - iOS data-only message (no notification payload) → show local notification
    // - Android → always show local notification (system doesn't auto-show for foreground)
    if (Platform.isIOS && message.notification != null) {
      print('📩 iOS foreground: AppDelegate willPresent will show with sound automatically');
    } else {
      print('📩 Showing local notification (data-only or Android)');
    ...
    print('✅ Foreground notification processed successfully');
  }

  /// Handle notification tap from FCM (app in background)
  static void _handleNotificationTap(RemoteMessage message) {
    print('🔔 Notification tapped from background!');
    print('🔔 Data: ${message.data}');
    // Wait for navigator to be fully ready after app resumes from background
  ...
  static Future<void> _navigateToScreen(Widget screen) async {
    // Retry up to 10 times (3 seconds total) waiting for navigator to be ready
    for (int i = 0; i < 10; i++) {
      ...
      print('⏳ Navigator not ready yet, retrying... ($i)');
      await Future.delayed(const Duration(milliseconds: 300));
    }
    print('❌ Navigator still not available after retries');
  }
